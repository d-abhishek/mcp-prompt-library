...
def get_available_prompts():
    """Get list of available prompts for MCP prompts/list response"""
    prompt_files = list(prompts_dir.glob("*.md"))
    prompts = []
    
    for prompt_file in prompt_files:
        try:
            post = frontmatter.load(str(prompt_file))
            metadata = post.metadata
            
            if 'name' not in metadata:
                metadata['name'] = prompt_file.stem
            
            prompts.append({
                "name": metadata.get('name', prompt_file.stem),
...
def execute_prompt(prompt_name: str, args: dict) -> str:
    ...
    try:
        ...
        raise Exception(f"Error executing prompt {prompt_name}: {str(e)}")